...
void RenderingEngine::setBufferSize(vector2I size)
{
...
void RenderingEngine::renderMS(GLuint render_target, Camera* camera, Transform& camera_transform)
{
	vector2I ms_buffer_size = ms_g_buffer->getBufferSize();
	vector2I g_buffer_size = g_buffer->getBufferSize();

	this->generateGBuffer(ms_g_buffer, camera, camera_transform);
	...
	this->drawAmbient(render_target, g_buffer, camera, camera_transform);
}

void RenderingEngine::render(GLuint render_target, Camera* camera, Transform& camera_transform)
{
	this->generateGBuffer(g_buffer, camera, camera_transform);
	this->drawAmbient(render_target, g_buffer, camera, camera_transform);
}

void RenderingEngine::addModel(Model* model, Transform global_transform)
{
...
void RenderingEngine::addPointLight(PointLight* point, Transform global_transform)
{
	this->point_lights.push_back({ point, global_transform });
}

void RenderingEngine::addSpotLight(SpotLight* spot, Transform global_transform)
{
...
void RenderingEngine::setSkybox(Skybox* box)
{
	this->skybox = box;
}

void RenderingEngine::setAmbientLight(vector3F ambient_light)
{
...
void RenderingEngine::generateGBuffer(G_Buffer* g_buffer, Camera* camera, Transform& camera_transform)
{
	vector2I g_buffer_size = g_buffer->getBufferSize();
	...
	{
		skybox->draw(camera, camera_transform, g_buffer_size);
...
void RenderingEngine::drawAmbient(GLuint render_target, G_Buffer* g_buffer, Camera* camera, Transform& camera_transform)
{
	...
	{
		this->drawLights(render_target, g_buffer, camera, camera_transform);
	}
...
void RenderingEngine::drawLights(GLuint render_target, G_Buffer* g_buffer, Camera* camera, Transform& camera_transform)
{
	glBindFramebuffer(GL_FRAMEBUFFER, render_target);
...
void RenderingEngine::drawShadowMap(ShadowMap* target, vector3D& camera_position, matrix4& light_space_matrix)
{
